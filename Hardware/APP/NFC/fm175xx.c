...
uint8_t SPIRead(uint8_t addr)
{
	...
	SpiReadWrite(NFC_SPI, send_data);
	reg_value = YA_SpiReadData(NFC_SPI);
...
uint8_t Set_Rf(unsigned char mode) 
{
    ...
    vTaskDelay(200);//打开TX输出后需要延时等待天线载波信号稳定
                       ...
                       unsigned int *pOutLenBit) 
{
    ...
    while (1)//循环判断中断标识
    {
        ...
        //LOG("command = 0x%2X, irq = 0x%2X, InLenByte=%d.\r\n", Command, irq, InLenByte);
...
{   
    if(FM175XX_HardReset() == OK) 
    {
        ...
        SystemInfo.nfcCardState = 1;
        LOG("FM175XX_Init OK.\r\n");
        return OK;
    }
        
    SystemInfo.nfcCardState = 0;             
	LOG("FM175XX_Init error.\r\n");
    return FAIL;
}
